...
func (h Server) Load(router fiber.Router) fiber.Router {
	...
	router.Get("/", h.adminRoute(config.Roles.ViewNotifyLogsByRecipient), h.wrapWithTimeout(h.GetAllByRecipient))
	router.Get("/admin", h.adminRoute(config.Roles.ViewNotifyLogsByChannel), h.wrapWithTimeout(h.GetAllByChannel))
	router.Get("/:uuid", h.adminRoute(config.Roles.ViewNotifyLogDetail), h.wrapWithTimeout(h.GetByUUID))
	return router
}
